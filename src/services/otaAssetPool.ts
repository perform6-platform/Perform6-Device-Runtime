...
function assetNameForPath(relPath: string): string {
  // Asset names may not contain slashes. Preserve a deterministic unique name;
  // the activation map restores the package's relative path afterwards.
...
function copyRealizedPackageToActive(
  ...
  try {
    for (const file of files) {
      ...
      if (!fs.existsSync(staged)) {
        throw new Error(`OTA staged file missing: ${rel}`);
      }
...
export async function installOtaViaAssetPool(
  ...
  try {
    ...
    const realized = copyRealizedPackageToActive(
      files,
      stageRoot,
      targetVersion,
    );

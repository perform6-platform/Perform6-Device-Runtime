...
function assertAutorunPlayerAllocation(autorunPath) {
  ...
  if (text.includes('1920x1080x60p:fullres') || text.includes('1920x1080x60p"')) {
    fail('autorun.brs still locks 1080p fleet output — expected 4K60');
  }

  console.log(
    '[release:zip] autorun player assert OK (XT HDMI-2×1, XC HDMI-2×1 HDMI-3×1, no pre-HTML LED, 4K60)',
  );
}

/**
 * Guard: XT+XC LED = BA bridge + SD fallback; AssetRealizer .mp4 playback.
 */
function assertLedPlaybackBus(autorunPath) {
  ...
  if (!fs.existsSync(jsLed)) fail('src/platform/ledPlaybackFile.ts missing');
  const ledText = fs.readFileSync(jsLed, 'utf8');
  ...
  console.log('[release:zip] LED SD bus assert OK (realized .mp4, BA bridge, JS write)');
}
...
function main() {
    ...
    JSON.stringify(
      {
        ...
        mediaCache: {
          ...
          naming: 'stable .mp4 names realized from AssetPool objects',
          usedBy: ['native roVideoPlayer LEDs via PlayFile', 'JS led-playback SD bus'],
          backingPath: 'SD:/perform6-media-pool (sha256 objects; not the native playback target)',
        },
        ...
        multiHdmi:
          profileKey === 'XT2145'
            ? {
                ...
                outputMap: 'HDMI-1 x=0; HDMI-2 x=3840',
  ...
  fs.writeFileSync(
    ...
    [
      ...
      '  MULTI           = 3840x2160x60p:fullres (recommended; BA-style dual/triple canvas)',
      ...
      profileKey === 'XT2145'
        ...
        : profileKey === 'XC4055'
          ? 'HDMI-1 React; HDMI-2/3 LEDs play realized .mp4 files via autorun PlayFile.'
          : 'Each HD226 player caches assigned media and plays local file:// where available.',
      profileKey === 'XT2145'

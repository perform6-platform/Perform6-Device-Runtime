...
Function ShouldEmitTrace(msg as String) as Boolean
  ...
  if Instr(1, msg, "PLAY|maintenance-skip") > 0 then return true
...
End Sub
Sub HandleLedCacheClearAll(payload as Object, states as Object)
  g = GetGlobalAA()
  g.p6MediaMaintenance = true
  EnsureLedIdleForStates(states)
  DeleteFile("SD:/perform6-led-playback.json")
  DeleteFile("SD:/perform6-xt-playback.json")
  if PayloadString(payload, "nodeWipe") <> "true" then ScheduleDeferredMediaWipe()
  requestId = PayloadString(payload, "requestId")
  AtomicWriteAsciiFile("SD:/perform6-media-clear-ready.txt", requestId + "|1")
  LedLog("=== Perform6: media clear prepared (idle; OTA untouched) ===")
  FlushLedLog()
  ack = CreateObject("roAssociativeArray")
  ack.AddReplace("type", "led-cache-clear-ready")
  ack.AddReplace("requestId", requestId)
  ack.AddReplace("ok", "1")
  PostJsMessage(ResolveBridgeHtml(states), ack)
End Sub
Sub HandleLedCacheClearResume(payload as Object, states as Object)
  g = GetGlobalAA()
  g.p6MediaMaintenance = false
  for each st in states
    if type(st) = "roAssociativeArray" then g.AddReplace("p6PbSig_" + st.key, "")
  end for
  LedLog("=== Perform6: media clear transaction resumed playback ===")
  requestId = PayloadString(payload, "requestId")
  AtomicWriteAsciiFile("SD:/perform6-media-clear-resumed.txt", requestId + "|1")
  ack = CreateObject("roAssociativeArray")
  ack.AddReplace("type", "led-cache-clear-resumed")
  ack.AddReplace("requestId", requestId)
  ack.AddReplace("ok", "1")
  PostJsMessage(ResolveBridgeHtml(states), ack)
End Sub
...
Sub ApplyNativePlayback(st as Object, payload as Object, msgPort as Object, states as Object)
  ...
  g = GetGlobalAA()
  if g.p6MediaMaintenance = true then
    TraceLog("PLAY|maintenance-skip|ApplyNativePlayback")
    return
  end if
  if type(st.vp) <> "roVideoPlayer" then
...
Sub MaybeResumePlaybackFromFile(states as Object, msgPort as Object, reason as String)
  ...
  g = GetGlobalAA()
  if g.p6MediaMaintenance = true then
    WriteLedBusHeartbeat("media-maintenance", reason)
    if TraceVerboseEnabled() then TraceFnExit("MaybeResumePlaybackFromFile", "media-maintenance")
    return
  end if
...
Sub ProcessOpsOnBoot(states as Object)
  ...
  if OpsJsonFieldTrue(content, "clearCacheOnBoot") then
    ...
    clearPayload = CreateObject("roAssociativeArray")
    clearPayload.AddReplace("nodeWipe", "false")
    HandleLedCacheClearAll(clearPayload, states)
        ...
        if reason = "load-error" then
          ...
          if type(payload) <> "roAssociativeArray" then
            ...
            if msgType = "led-bridge-ping" then
              ...
              HandleLedCacheClearAll(payload, ledStates)
            else if msgType = "led-cache-clear-resume" then
              HandleLedCacheClearResume(payload, ledStates)
